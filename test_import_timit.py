...
#        labels2 = []
        ...
        mfcc_feats1 = compute_mfcc(wav_file[durations[0][0]:durations[0][1]], n_delta=n_delta1)
        mfcc_feats2 = compute_mfcc(wav_file[durations[0][0]:durations[0][1]], n_delta=n_delta2)
        ...
        for index, chunk in enumerate(durations[1:]):
            ...
            mfcc_feat1 = compute_mfcc(wav_file[chunk[0]:chunk[1]], n_delta=n_delta1)
            mfcc_feats1 = np.vstack((mfcc_feats1, mfcc_feat1))

            mfcc_feat2 = compute_mfcc(wav_file[chunk[0]:chunk[1]], n_delta=n_delta2)
            mfcc_feats2 = np.vstack((mfcc_feats2, mfcc_feat2))
        ...
        mfcc_features1.extend(mfcc_feats1)
#        mfcc_labels1.extend(labels1)

        mfcc_features2.extend(mfcc_feats2)
#        mfcc_labels2.extend(labels2)
    ...
    timit_df1 = pd.DataFrame()
    timit_df1["features"] = mfcc_features1
    timit_df1["labels"] = mfcc_labels
    timit_df1.to_hdf("./test_features/mfcc_delta/timit.hdf", "timit")

    timit_df2 = pd.DataFrame()
    timit_df2["features"] = mfcc_features2
    timit_df2["labels"] = mfcc_labels
    timit_df2.to_hdf("./test_features/mfcc_delta_delta/timit.hdf", "timit")
